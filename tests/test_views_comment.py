...
class TestCommentViewSet:
    ...
    def test_retrieve(self, client, user, post, comment):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == str(comment.public_id.hex)
        assert response.data["body"] == comment.body
        assert response.data["author"]["public_id"] == str(comment.author.public_id)
    ...
    def test_create(self, client, user, post):
        ...
        assert response.data["author"]["public_id"] == str(user.public_id)
